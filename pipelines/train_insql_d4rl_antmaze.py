...
@hydra.main(config_path="../configs/insql/antmaze", config_name="antmaze", version_base=None)
def pipeline(args):
    ...
    actor = InsQL(
        ...
        x_min=-1. * torch.ones((1, act_dim), device=args.device), time_dist=[args.time_dist,-0.4,1],adaptive_l2_loss=args.adaptive_l2_loss,flow_ratio=args.flow_ratio, warm_up_steps=args.warm_up_steps,
    ...
    if args.mode == "train":
        ...
        for batch in loop_dataloader(dataloader):
            ...
            next_act, _ = actor.sample(
                ...
                n_samples=args.batch_size * 10, sample_steps=args.train_sampling_steps, use_ema=args.use_ema,
            ...
            if (n_gradient_step + 1) % args.eval_interval == 0:
                ...
                t1 = time.time() - t0
                print(f"Mean: {mean:.3f}, Error: {err:.3f}, Eval duration: {t1:.2f} seconds")
    ...
    # ---------------------- Inference ----------------------
        ...
        for i in range(args.num_episodes):
            ...
            while not np.all(cum_done) and t < 1000 + 1:
                # viewer.cam.distance = 60  # 摄像机距离观察点的距离
                # viewer.cam.elevation = -90  # 摄像机的仰角，-90度表示完全向下
                
                camera.cam.distance = 60  # 摄像机距离观察点的距离
                camera.cam.elevation = -90  # 摄像机的仰角，-90度表示完全向下
