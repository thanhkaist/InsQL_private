...
class BaseDiffusionSDE(DiffusionModel):
    ...
    # for distill dql only
    def loss_dql(self, x0, condition=None, weighted_tensor=None):
        ...
        if self.predict_noise:
            loss = (self.model["diffusion"].policy_forward(xt, t, condition) - eps) ** 2
        else:
            loss = (self.model["diffusion"].policy_forward(xt, t, condition) - x0) ** 2
    ...
    def guided_sampling_with_hidden(
            self, xt, t, alpha, sigma,
            model,
            condition_cfg=None, w_cfg: float = 0.0,
            condition_cg=None, w_cg: float = 0.0,
        ...
        assert w_cg == 0.0 and condition_cg is None, "Hidden are not supported in classifier-guidance."
        assert condition_cfg is None, "Hidden are not supported in CFG."
        
        with torch.set_grad_enabled(requires_grad):
            pred_hidden = model["diffusion"](xt, t, condition_cfg, output_hidden=True)
            
        return pred_hidden
    
    def guided_sampling_with_latents(
            self, xt, t, alpha, sigma,
            model,
            condition_cfg=None, w_cfg: float = 0.0,
            condition_cg=None, w_cg: float = 0.0,
        ...
        assert w_cg == 0.0 and condition_cg is None, "Latents are not supported in classifier-guidance."
        assert w_cfg == 1.0 and condition_cfg is not None, "Latents are only supported in full conditional in CFG."
        assert posterior is not None, "Posterior from planner is required for latents."
        
        with torch.set_grad_enabled(requires_grad):
            pred = model["diffusion"](posterior)
            policy_latents = model["diffusion"].get_policy_z(xt, t, condition_cfg)
            planner_latents = model["diffusion"].get_planner_z(posterior)
        
        latents = {
            "policy_latents": policy_latents,
            "planner_latents": planner_latents
        }
...
class DiscreteDiffusionSDE(BaseDiffusionSDE):
    ...
    def get_clean_t(self, x0):
        clean_t = torch.ones(x0.shape[0], device=self.device, dtype=torch.int64) * self.diffusion_steps
        return clean_t
    
    def get_alpha_sigma(self, x0, t):
    ...
    def sample_planner(
        ...
        with torch.set_grad_enabled(requires_grad):
            ...
            action = model["diffusion"].planner_forward(extra_inputs['posterior'], deterministic=deterministic)
    ...
    def sample(
        ...
        for i in reversed(loop_steps):
            ...
            if get_latents:
                assert "posterior" in extra_inputs, "posteriro from planner is required for latents."
                pred, latents = self.guided_sampling_with_latents(
                    xt, t, alphas[i], sigmas[i],
                    model, condition_vec_cfg, w_cfg, condition_vec_cg, w_cg, requires_grad, posterior=extra_inputs["posterior"])
            else:
                # guided sampling
            ...
            if solver == "ddpm":
                ...
                assert self.predict_noise is False, "RNN solver only supports data prediction."
                xt = x_theta
...
class ContinuousDiffusionSDE(BaseDiffusionSDE):
    ...
    def sample(
        ...
        for i in reversed(loop_steps): # [20, 19, 18, 17, ..., 1]
            ...
            if get_hidden:
                assert self.predict_noise is True, "Hidden sampling only supports noise prediction."
                pred_hidden = self.guided_sampling_with_hidden(
                    xt, t, alphas[i], sigmas[i],
                    model, condition_vec_cfg, w_cfg, condition_vec_cg, w_cg, requires_grad)
