...
from cleandiffuser.utils import GaussianNormalizer, EmptyNormalizer, dict_apply
...
class NormedD4RLMuJoCoDataset(BaseDataset):
...
class SparseNormedD4RLMuJoCoDataset(BaseDataset):
    """ **D4RL-MuJoCo Sequential Dataset**
    ...
    def __init__(
        ...
        assert horizon > 0 and (horizon & (horizon - 1)) == 0, f"{horizon} should be a power of 2"
        import math
        self.horizon_power = int(math.log2(horizon))
        self.horizon_index = np.array([int(pow(2,k))-1 for k in range(self.horizon_power + 1)])
        print(self.horizon_index)
    ...
    def __getitem__(self, idx: int):
        path_idx, start, end = self.indices[idx]
        current_index = self.horizon_index + start
        
        horizon_state = self.seq_obs[path_idx, current_index]

        data = {
            'obs': {'state': horizon_state},
            'act': self.seq_act[path_idx, current_index],
            'rew': self.seq_rew[path_idx, current_index],
            'val': self.seq_val[path_idx, start],
        }
...
class TTAwareD4RLMuJoCoDataset(BaseDataset):
    """ **D4RL-MuJoCo Sequential Dataset**
    ...
    def __init__(
            ...
            center_mapping: bool = True,
            stride: int = 1,
            truncted_estimation_length: int = 50,
        ...
        assert stride == 1, "Currently, we only support stride=1 for MuJoCo"
        ...
        n_paths = np.sum(np.logical_or(terminals, timeouts))
        self.seq_obs = np.zeros((n_paths+1, max_path_length + horizon, self.o_dim), dtype=np.float32)
        self.seq_act = np.zeros((n_paths+1, max_path_length + horizon, self.a_dim), dtype=np.float32)
        self.seq_rew = np.zeros((n_paths+1, max_path_length + horizon, 1), dtype=np.float32)
        self.seq_val = np.zeros((n_paths+1, max_path_length + horizon, 1), dtype=np.float32)
        self.indices = []
        ...
        for i in range(timeouts.shape[0]):
            if timeouts[i] or terminals[i]:
                ...
                if terminals[i]:
                    ...
                    self.seq_obs[path_idx, path_length:] = normed_observations[i]
                    self.seq_act[path_idx, path_length:] = actions[i]
                    self.seq_rew[path_idx, path_length:] = 0
                    ...
                    future_reward_pred = 0
                    look_back_length = min(path_length, truncted_estimation_length)
                    for j in reversed(range(i - look_back_length, i)):
                        future_reward_pred = future_reward_pred * discount + rewards[j]
                    future_reward_pred /= (1 - pow(discount, look_back_length))
                    
                    self.seq_rew[path_idx, path_length] = future_reward_pred
                    print(f"modifying the last reward with: {future_reward_pred}")
        ...
        if value_type == "return":
            ...
            for path_idx, start, end in self.indices:
                horizon_rewards = self.seq_rew[path_idx, start:end, 0]
                horizon_discount = discount ** np.arange(end-start)
                self.seq_val[path_idx, start] = np.sum(horizon_rewards * horizon_discount)
...
class D4RLMuJoCoDataset(BaseDataset):
    ...
    def __init__(
        ...
        observations, actions, next_observations, rewards, timeouts, terminals = (
            dataset["observations"].astype(np.float32),
            dataset["actions"].astype(np.float32),
