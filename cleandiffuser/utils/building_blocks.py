...
class JoshMLP(nn.Module):
    def __init__(
        ...
        assert len(dims) > 1, "dims must have at least 2 elements"
...
class NormalCritic(nn.Module):
    """ **Deep Q-Learning Critic.** A pytorch critic module for Q. The module incorporates double Q trick.
    ...
    def __init__(self, obs_dim: int, act_dim: int, hidden_dim: int = 256):
        ...
        self.q_model = nn.Sequential(
            nn.Linear(obs_dim + act_dim, hidden_dim), nn.SiLU(),
...
class DAHorizonCritic(nn.Module):
    ...
    def forward(self, x: torch.Tensor, get_hidden=False):
        """
        Input:
            x:          (b, horizon, in_dim)
        ...
        Output:
            y:          (b, horizon, in_dim)
        """
        assert len(x.shape) == 3, f"Input shape must be (b, horizon, in_dim), got {x.shape}"
